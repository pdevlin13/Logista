...
# file. This is a work in progress with a lots more required on error handling
# and program config options. The code is written to fulfill the
...
# have a play with Pythonista and want a starting point.
...
class logform(ui.View):          # initialise the variable associated with
	def __init__(self):       # with each object in the ui grphics file
		...
		# setup filenames to use
		adifchk = False
		i = 0
		while adifchk is not True:
			adifname = icloud_path + 'logista' + str(i) + '.adi'
		...
		while sotafchk is not True:
			sotafname = icloud_path + 'sota' + str(i) + '.csv'
